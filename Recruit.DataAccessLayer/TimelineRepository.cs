       ...
       public void Delete(Timeline entity)
        ...
        public List<Timeline> GetAll()
        {
            ...
            try
            {
                ...
                {
                    data = database.Query<Timeline>("SELECT tbl_interview_details.id, tbl_process_stages.stage as 'stage', tbl_interview_details.candidate_id, first_name, start_date_time, end_date_time, status_id, status, reason, tbl_interviewers.employee_id, tbl_employees.name as 'emp_name' FROM tbl_interview_details INNER JOIN tbl_candidates ON  tbl_interview_details.candidate_id = tbl_candidates.id INNER JOIN tbl_interview_round_statuses ON  tbl_interview_details.status_id = tbl_interview_round_statuses.id LEFT JOIN tbl_interviewers ON   tbl_interview_details.id = tbl_interviewers.interview_id LEFT JOin tbl_employees ON tbl_interviewers.employee_id = tbl_employees.id LEFT JOin tbl_process_stages ON tbl_interview_details.interview_stage_id = tbl_process_stages.id order by(start_date_time)").ToList();
                }
            }
